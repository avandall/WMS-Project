...
from PMKT.module.custom_exceptions import (
    ValidationError, BusinessRuleViolationError, EntityNotFoundError,
    InvalidDocumentStatusError, InvalidQuantityError, InvalidIDError
)
...
class TestDocumentProduct:
    ...
    def test_document_product_invalid_unit_price(self, invalid_price):
        ...
        with pytest.raises(InvalidQuantityError, match="Unit price must be non-negative"):
...
class TestDocument:
    ...
    def test_document_invalid_id(self, invalid_id):
        ...
        with pytest.raises(InvalidIDError, match="Document ID must be a positive integer"):
            Document(document_id=invalid_id, doc_type=DocumentType.IMPORT, to_warehouse_id=1, created_by="user")
    ...
    def test_document_invalid_type(self):
        ...
        with pytest.raises(ValidationError, match="Invalid document type"):
            Document(document_id=1, doc_type="INVALID", to_warehouse_id=1, created_by="user")

    def test_document_import_missing_destination(self):
        """Test creating import document without destination warehouse."""
        with pytest.raises(ValidationError, match="Import document must have a destination warehouse"):
    ...
    def test_document_export_missing_source(self):
        ...
        with pytest.raises(ValidationError, match="Export document must have a source warehouse"):
    ...
    def test_document_transfer_missing_warehouses(self):
        ...
        with pytest.raises(ValidationError, match="Transfer document must have both source and destination warehouses"):
            Document(document_id=1, doc_type=DocumentType.TRANSFER, from_warehouse_id=1, created_by="user")

    def test_document_transfer_same_warehouse(self):
        """Test creating transfer document with same source and destination."""
        with pytest.raises(BusinessRuleViolationError, match="Transfer document cannot have same source and destination warehouse"):
            Document(document_id=1, doc_type=DocumentType.TRANSFER, from_warehouse_id=1, to_warehouse_id=1, created_by="user")
    ...
    def test_document_invalid_created_by(self, invalid_created_by):
        ...
        with pytest.raises(ValidationError, match="Created by cannot be empty"):
            Document(document_id=1, doc_type=DocumentType.IMPORT, to_warehouse_id=1, created_by=invalid_created_by)

    def test_add_item_valid(self):
        """Test adding an item to document."""
        document = Document(document_id=1, doc_type=DocumentType.IMPORT, to_warehouse_id=1, created_by="user")
    ...
    def test_add_item_duplicate_product(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Product 1 already exists in document"):
    ...
    def test_add_item_posted_document(self):
        ...
        with pytest.raises(InvalidDocumentStatusError, match="Cannot modify document 1 that is not in DRAFT status"):
    ...
    def test_remove_item_not_found(self):
        ...
        with pytest.raises(EntityNotFoundError, match="Product 1 not found in document"):
            document.remove_item(product_id=1)
    ...
    def test_post_document_no_items(self):
        ...
        with pytest.raises(BusinessRuleViolationError, match="Cannot post document without items"):
            document.post("approver")
    ...
    def test_post_document_already_posted(self):
        ...
        with pytest.raises(InvalidDocumentStatusError, match="Document 1 is not in DRAFT status"):
    ...
    def test_cancel_document_posted(self):
        ...
        with pytest.raises(InvalidDocumentStatusError, match="Cannot cancel a posted document 1"):
            document.cancel()
    ...
    def test_equality(self):
        ...
        document1 = Document(document_id=1, doc_type=DocumentType.IMPORT, to_warehouse_id=1, created_by="user")
        document2 = Document(document_id=1, doc_type=DocumentType.EXPORT, from_warehouse_id=1, created_by="user")
        document3 = Document(document_id=2, doc_type=DocumentType.IMPORT, to_warehouse_id=1, created_by="user")
