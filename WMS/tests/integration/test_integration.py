...
def test_product_crud():
    ...
    # Create products
    products = [
        {"product_id": 101, "name": "Laptop Dell XPS 15", "price": 1500.00, "description": "High-end laptop"},
        {"product_id": 102, "name": "Mouse Logitech MX Master", "price": 99.99, "description": "Wireless mouse"},
        {"product_id": 103, "name": "Keyboard Mechanical", "price": 150.00, "description": "RGB keyboard"}
    ]
...
def test_import_document_flow():
    ...
    # Post the document
    response = requests.post(f"{BASE_URL}/documents/{doc_id}/post", json={"approved_by": "manager"})
    ...
    if assert_status(response, 200, "Get warehouse after import"):
        ...
        wh_prod_101 = next((item for item in wh_inventory if item["product_id"] == 101), None)
        if wh_prod_101:
...
def test_export_document_flow():
    ...
    export_doc = {
        "warehouse_id": warehouse_id,
        "items": [
            {"product_id": 102, "quantity": 5, "unit_price": 99.99}
        ],
        "created_by": "admin",
...
def test_transfer_document_flow():
    ...
    transfer_doc = {
        ...
        "items": [
            {"product_id": 101, "quantity": 3, "unit_price": 1450.00}
        ],
        "created_by": "admin",
    ...
    if assert_status(response, 200, "Get source warehouse after transfer"):
        ...
        prod_101_before = next((item for item in wh_before["inventory"] if item["product_id"] == 101), None)
        prod_101_after = next((item for item in wh_after["inventory"] if item["product_id"] == 101), None)
        
        if prod_101_before and prod_101_after:
            assert_equal(prod_101_after["quantity"], prod_101_before["quantity"] - 3, 
    ...
    if assert_status(response, 200, "Get destination warehouse after transfer"):
        ...
        prod_101_dest = next((item for item in wh_dest["inventory"] if item["product_id"] == 101), None)
        if prod_101_dest:
            assert_equal(prod_101_dest["quantity"], 3, "Destination warehouse received items")
...
def test_error_validations():
    ...
    import_doc = {
        "warehouse_id": test_data["warehouses"][0]["warehouse_id"] if test_data["warehouses"] else 1,
