...
from ..module.custom_exceptions import InvalidQuantityError, WarehouseNotFoundError, ProductNotFoundError, InsufficientStockError
...
class Warehouse:
    ...
    def _validate_warehouse_id(warehouse_id: int) -> None:
        ...
        if not isinstance(warehouse_id, int) or warehouse_id <= 0:
            raise InvalidQuantityError("Warehouse ID must be a positive integer")
    ...
    def _validate_location(location: str) -> None:
        ...
        if not location or not isinstance(location, str) or len(location.strip()) == 0:
            raise ValidationError("Warehouse location cannot be empty")
        if len(location) > 200:
            raise ValidationError("Warehouse location cannot exceed 200 characters")
    ...
    def remove_product(self, product_id: str, quantity: int) -> None:
            ...
            if item.product_id == product_id:
                if item.quantity < quantity:
                    raise InsufficientStockError(
                        f"Insufficient stock in warehouse {self.warehouse_id}. "
        ...
        raise ProductNotFoundError(f"Product {product_id} not found in warehouse {self.warehouse_id}")
