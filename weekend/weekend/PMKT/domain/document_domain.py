...
from ..module.custom_exceptions import ValidationError, BusinessRuleViolationError, EntityNotFoundError
...
from ..module.custom_exceptions import InvalidDocumentStatusError, InvalidQuantityError
...
class DocumentProduct:
    ...
    def _validate_product_id(product_id: int) -> None:
        ...
        if not isinstance(product_id, int) or product_id <= 0:
            raise InvalidQuantityError("Product ID must be a positive integer")
    ...
    def _validate_unit_price(unit_price: float) -> None:
        ...
        if not isinstance(unit_price, (int, float)) or unit_price < 0:
            raise InvalidQuantityError("Unit price must be non-negative")
...
class Document:
    ...
    def _validate_document_id(document_id: int) -> None:
        ...
        if not isinstance(document_id, int) or document_id <= 0:
            raise InvalidQuantityError("Document ID must be a positive integer")
    ...
    def _validate_warehouses(doc_type: DocumentType, from_warehouse_id: Optional[int], to_warehouse_id: Optional[int]) -> None:
        ...
        if doc_type == DocumentType.IMPORT:
            if to_warehouse_id is None:
                raise ValidationError("Import document must have a destination warehouse")
        elif doc_type == DocumentType.EXPORT:
            if from_warehouse_id is None:
                raise ValidationError("Export document must have a source warehouse")
        elif doc_type == DocumentType.TRANSFER:
            if from_warehouse_id is None or to_warehouse_id is None:
                raise ValidationError("Transfer document must have both source and destination warehouses")
            if from_warehouse_id == to_warehouse_id:
                raise BusinessRuleViolationError("Transfer document cannot have same source and destination warehouse")
    ...
    def _validate_created_by(created_by: str) -> None:
        ...
        if not created_by or not isinstance(created_by, str) or len(created_by.strip()) == 0:
            raise ValidationError("Created by cannot be empty")
