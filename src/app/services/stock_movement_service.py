...
from app.application.ports import IAuditEventRepo, IPositionRepo, IWarehouseRepo
...
class StockMovementService:
    """
    Position-aware stock movement service.

    Supports:
    - Put-away (RECEIVING -> STORAGE or any target)
    - Picking (STORAGE -> SHIPPING/STAGING or any target)
    - Generic position move inside a warehouse
    - Warehouse-to-warehouse transfer with positions
    """
    ...
    def move_within_warehouse(
        ...
        *,
        ...
        # Ensure default bins exist and reconcile UNASSIGNED against warehouse totals
        self._ensure_defaults_and_balance(warehouse_id, product_id)

        try:
            self._set_repos_auto_commit(False)

            from_pos = self.position_repo.get_position_model(warehouse_id, from_position_code)  # type: ignore[attr-defined]
            to_pos = self.position_repo.get_position_model(warehouse_id, to_position_code)  # type: ignore[attr-defined]

            self.position_repo.adjust_position_stock(  # type: ignore[attr-defined]
            ...
            logger.info(
                f"Moved stock within warehouse {warehouse_id}: product_id={product_id} qty={quantity} {from_pos.code}->{to_pos.code}"
            )
            return {
                "warehouse_id": warehouse_id,
                "product_id": product_id,
    ...
    def transfer_between_warehouses(
        ...
        *,
        ...
        try:
            ...
            to_pos = self.position_repo.get_position_model(to_warehouse_id, to_position_code)  # type: ignore[attr-defined]

            # Position-level removal with allocation (more robust than forcing a single bin).
            preferred = []
            for code in [
                from_position_code,
                "SHIPPING",
            ...
            allocations = self.position_repo.allocate_and_remove(  # type: ignore[attr-defined]
            ...
            # Warehouse-level totals (global inventory unchanged)
            ...
            logger.info(
                f"Transferred stock: product_id={product_id} qty={quantity} wh{from_warehouse_id} -> wh{to_warehouse_id}:{to_pos.code}"
            )
            return {
                "product_id": product_id,
                "quantity": quantity,
                "from": {
                    "warehouse_id": from_warehouse_id,
                    "allocations": [{"position": code, "quantity": qty} for code, qty in allocations],
                },
                "to": {
                    "warehouse_id": to_warehouse_id,
                    "position": to_pos.code,
    ...
    def _ensure_defaults_and_balance(self, warehouse_id: int, product_id: int) -> None:
        ...
        pos_total = self.position_repo.get_total_quantity_for_product(warehouse_id, product_id)  # type: ignore[attr-defined]
        diff = wh_qty - pos_total
        if diff == 0:
            return

        # Reconcile by adjusting UNASSIGNED to match warehouse totals.
        unassigned = self.position_repo.get_position_model(warehouse_id, "UNASSIGNED")  # type: ignore[attr-defined]
        if diff > 0:
            self.position_repo.adjust_position_stock(  # type: ignore[attr-defined]
                position_id=unassigned.id, product_id=product_id, delta=diff
            )
        else:
            # diff < 0: positions have more than warehouse totals (data corruption)
            # Try to remove from UNASSIGNED first; if not enough, raise.
            try:
                self.position_repo.adjust_position_stock(  # type: ignore[attr-defined]
                    position_id=unassigned.id, product_id=product_id, delta=diff
                )
