...
class DocumentCreate(BaseModel):
    doc_type: Optional[str] = Field(
        None, description="Document type: import, export, or transfer"
    )
    warehouse_id: Optional[int] = Field(
        None, gt=0, description="Legacy warehouse ID field for import/export requests"
    )
    to_warehouse_id: Optional[int] = Field(
        None, gt=0, description="Legacy alias for destination warehouse ID"
    )
    from_warehouse_id: Optional[int] = Field(
        None, gt=0, description="Legacy alias for source warehouse ID"
    )
    ...
    def normalize_legacy_fields(cls, data):
        ...
        # Legacy aliases used by older tests/clients
